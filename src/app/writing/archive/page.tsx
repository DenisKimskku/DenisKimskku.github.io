    ...
    <div className="container-custom py-16 md:py-24">
      ...
      <header className="mb-12">
        ...
        <p className="mt-2 text-xs text-(--color-text-muted)">
          Entries marked{' '}
          <span className="px-1 py-px rounded-sm border border-(--color-border) text-[10px] uppercase tracking-wider">
            auto
          </span>{' '}
      ...
      <div className="space-y-10">
        {years.map((year) => (
          <section key={year}>
            <h2 className="text-2xl font-semibold font-serif text-(--color-text) mb-4">
              {year}
            </h2>
            <div className="space-y-1">
              {articlesByYear.get(year)!.map((article) => (
                <Link
                  ...
                  className="group flex items-baseline gap-4 py-2.5 -mx-4 px-4 rounded-lg hover:bg-(--color-bg-secondary) transition-colors"
                >
                  <time
                    dateTime={article.date}
                    className="text-sm text-(--color-text-muted) tabular-nums shrink-0 w-22"
                  >
                    {article.date.slice(5)}
                  </time>
                  <div className="flex-1 min-w-0">
                    <span className="text-(--color-text) group-hover:text-(--color-accent) transition-colors font-serif">
                      {article.title}
                    </span>
                    <span className="ml-2">
