...
The physical and side-channel vulnerabilities inherent to localized deployments are covered in [Securing LLMs in the Wild: Privacy and Security Challenges at the Edge](/writing/securing_llms_in_the_wild_privacy_and_security_challenges_at), which is directly relevant here because it highlights how moving computation to the edge often trades centralized security for unmonitored physical exposure. FLINT shows that even when training data never leaves the device, radio emissions can still betray its architecture.