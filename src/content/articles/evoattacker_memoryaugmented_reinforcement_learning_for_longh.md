...
The notable contribution of Evo-Attacker is that it moves past simplistic, static prompt injections to target the glaring Achilles' heel of multi-agent systems: the implicit trust in tool outputs. In the rush to deploy autonomous agent teams—like Devin-style software engineering loops—user prompts have been heavily secured while the backend is left wide open. 
...
This paper operationalizes a hazard covered in the earlier piece on [Security of Autonomous AI Agents: Trust Boundary-Based Attack Surface Analysis and Trends](/writing/security_autonomous_ai_agents_trust_boundary), which argued that treating tool outputs as trusted ground truth creates an undefended boundary that adversaries can easily exploit. For teams building agentic workflows today, external API returns can no longer be treated as safe zones.