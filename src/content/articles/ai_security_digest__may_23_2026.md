---
title: "AI Security Digest — May 23, 2026: RAG Retrieval Corruption & Agent Memory Poisoning"
date: "2026-05-23"
type: "News Digest"
...
Academic research is finally catching up to what practitioners face in production: the era of static, stateless AI is over. In one recent \$75M enterprise deployment of an autonomous procurement network, the core security failure was not a direct prompt injection, but rather a slow-drip poisoning of the company's shared internal knowledge graph. The most notable entry in this digest is RADAR's approach to RAG corruption. For too long, enterprise AI architectures have treated retrieved data as inherent ground truth—a dangerous, naïve assumption. 

When an LLM is connected to a dynamic database or the open web, the environment itself becomes the threat vector. This exact mechanism is covered in [AI Agent Traps: When the Environment Becomes the Attacker](/writing/ai_agent_traps), which details how indirect injection attacks can hijack an agent's control loop when it processes external, untrusted web content. Adversaries don’t need to spend multi-million \$ budgets on complex jailbreaks or weight tampering if they can simply poison the search index that a RAG pipeline blindly trusts. 

The EnCAgg paper on Federated Learning reinforces the same fundamental lesson. Defending against slow-drip, oscillating model poisoning requires dropping static thresholds. As covered in [Security of Autonomous AI Agents: Trust Boundary-Based Attack Surface Analysis and Trends](/writing/security_autonomous_ai_agents_trust_boundary), which establishes that autonomous systems must explicitly map and validate untrusted input boundaries, traditional security perimeters have completely dissolved. Whether it is a gradient update from a decentralized node or a retrieved document in a RAG pipeline, a zero-trust mindset is required for all external AI inputs. Structural, dynamic defenses like the ones highlighted today are not merely "nice to have"—they are mandatory for securing modern, stateful AI systems.