...
The most consequential aspect of this paper is the paradigm shift: the field is moving past human researchers hand-crafting heuristic search algorithms toward LLM-driven frameworks like OpenEvolve and GigaEvo that write the optimization code directly. Traditional zero-order optimizations struggle with the sharp, discrete cliffs of quantized perceptual hashes. Evolving search programs specifically tailored to bypass these landscapes yielded highly impressive results, dropping the composite attack cost score relative to optimized seeds by 41.2% for NeuralHash, 38.3% for PDQ, and 34.0% for pHash.

The real-world utility of these attacks warrants closer scrutiny, however. The query budgets here are massive—ranging from 10,000 queries for pHash up to 30,000 for NeuralHash. While mathematically sound, any modern trust-and-safety pipeline worth its salt will rate-limit or block an IP hammering their hash oracle with tens of thousands of sequential, slightly mutated images. Moving from a neat lab demo to a practical threat against commercial systems would require these evolved programs to achieve similar collision rates with a fraction of the queries.