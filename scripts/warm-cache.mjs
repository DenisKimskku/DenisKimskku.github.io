...
import { orderPurgeTargets } from './lib/cache-purge.mjs';

// /og/<slug>.png is regenerated by every build (scripts/generate-og-images.mjs)
// but only the newest cards realistically change (title/tag edits happen to
// recent posts); older ones ride out the 7-day edge TTL from the /og/ cache
// rule. Purging all ~270 would add three purge batches per deploy for nothing.
const OG_PURGE_NEWEST = Math.max(0, Number(process.env.CACHE_PURGE_OG_NEWEST ?? 40));
const OG_WARM_NEWEST = Math.max(0, Number(process.env.CACHE_WARM_OG_NEWEST ?? 12));
...
// Every feed the deploy job announces via WebSub must also be purged here —
// with the 30-day edge TTL from the cache rule, an un-purged feed would
// otherwise serve last month's items to subscribers.
function buildSiteDocumentTargets() {
  return [
    new URL('/sitemap.xml', baseUrl).toString(),
    new URL('/robots.txt', baseUrl).toString(),
    new URL('/rss.xml', baseUrl).toString(),
    new URL('/writing/rss.xml', baseUrl).toString(),
    new URL('/news/rss.xml', baseUrl).toString(),
  ];
}

function newestArticleSlugs(count) {
  if (count <= 0) {
    return [];
  }
...
function buildOgTargets(slugs) {
  return slugs.map((slug) => new URL(`/og/${encodeURIComponent(slug)}.png`, baseUrl).toString());
}

// The site card (_site.png), section cards (_section-*.png) and topic-hub cards
// (_tag-*.png) are referenced from og:image on every non-article page. They
// only change when the template version in generate-og-images.mjs is bumped,
// but when they do, a stale copy would otherwise ride out the 7-day /og/ TTL
// on the most-shared URLs on the site. They are ~20 files: always purge and
// warm them. public/og is git-tracked, so this works in the no-build CI jobs.
function buildStaticOgCardTargets() {
  ...
  return fs
    .readdirSync(ogDir)
    .filter((name) => name.startsWith("_") && name.endsWith(".png"))
    .sort()
    .map((name) => new URL(`/og/${encodeURIComponent(name)}`, baseUrl).toString());
}
...
function buildStaticAssetTargets() {
  return [
    ...
    // og:image is referenced from <meta content=...>, which the href/src
    // scanners never see — so social unfurls of fresh posts always paid a
    // cold-origin fetch. Warm the newest cards explicitly.
    ...buildOgTargets(newestArticleSlugs(OG_WARM_NEWEST)),
    ...buildStaticOgCardTargets(),
  ];
}
...
function buildPurgeTargets(pageUrls) {
  ...
  // Stamped with the commit SHA by the build job; humans check it through the
  // edge, so it must not sit behind a month-long TTL.
  targets.add(new URL('/deploy-marker.txt', baseUrl).toString());
  // The 404 page template is part of the build too.
  targets.add(new URL('/404.html', baseUrl).toString());

  const newestSlugs = newestArticleSlugs(OG_PURGE_NEWEST);
  for (const ogUrl of [...buildOgTargets(newestSlugs), ...buildStaticOgCardTargets()]) {
    targets.add(ogUrl);
  }
  ...
  // Ordered so the first purge batches (and therefore the first re-warms)
  // are the homepage, listings, feeds and the newest articles.
  return orderPurgeTargets(Array.from(targets), newestSlugs);
}
